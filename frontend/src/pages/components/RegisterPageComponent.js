...
const RegisterPageComponent = ({
  ...
  const emailHasEdu = (email, name, lastName, password) => {
    const regex = /@.*edu/gi;
    const hasEdu = regex.test(email);
    if (hasEdu) {
      axios.post("/sendOTP", {
        email: email,
      });
      navigate("/seller/verification", {
        state: { id: 1, email, name, lastName, password },
      });
  ...
  const handleSubmit = (event) => {
    ...
    if (
      ...
      if (!emailHasEdu(email, name, lastName, password)) {
